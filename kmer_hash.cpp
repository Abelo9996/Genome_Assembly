...
int main(int argc, char** argv) {
    ...
    size_t local_hash_table_size = ceil(1.0*hash_table_size/upcxx::rank_n());
    HashMap hashmap(hash_table_size, local_hash_table_size);

    upcxx::atomic_domain<int> atom_op_domain({upcxx::atomic_op::fetch_add});
    ...
    for (auto& kmer : kmers) {
        bool success = hashmap.insert(kmer, atom_op_domain);
        if (!success) {
